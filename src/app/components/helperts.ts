...
export function onGenerationComplete(response: GenerationResponse) {
  ...
  response.imageArtifacts.forEach((artifact) => {
    try {
      ...
      imgElement.src = `data:image/png;base64,${artifact.getBinary_asB64()}`;
  
      // Append the image to a container in your HTML (make sure to have a container with id 'image-container')
      ...
      console.error("Failed to display the resulting image", error);
    }
  });
